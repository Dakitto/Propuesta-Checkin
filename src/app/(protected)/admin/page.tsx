...
export default function AdminPage() {
  ...
  const handleCsvUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    importCounselors(parsedCounselors);
        ...
        <section className="grid gap-6 lg:grid-cols-[1.2fr,0.8fr]">
          <form onSubmit={handleConfigSubmit} className="space-y-5 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Cambia el nombre del sitio, la imagen de fondo y las credenciales de acceso.
              </p>
            </div>
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <label className="block">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setConfigForm((previous) => ({ ...previous, username: event.target.value }))
                  }
                  className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900"
                />
              </label>
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setConfigForm((previous) => ({ ...previous, password: event.target.value }))
                  }
                  className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900"
                />
              </label>
            ...
            <div className="rounded-[2rem] border border-dashed border-slate-300 bg-slate-50 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">Se guarda como base64 en localStorage.</p>
                </div>
              </div>
          ...
          <aside className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-6 rounded-[2rem] bg-slate-900 p-6 text-white">
              ...
              <p className="mt-2 text-sm text-indigo-100">Usuario activo: {configForm.username || "Sin usuario"}</p>
            </div>
          </aside>
        </section>
      ) : null}
                ...
                <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white text-slate-700">
                    {counselors.map((counselor) => (
                      <tr key={counselor.id}>
                        <td className="px-6 py-4">{counselor.iglesia}</td>
                        <td className="px-6 py-4 font-medium text-slate-900">{counselor.nombre}</td>
                        <td className="px-6 py-4">{counselor.telefono || "—"}</td>
                        <td className="px-6 py-4">{counselor.edad ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
